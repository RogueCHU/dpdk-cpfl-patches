...
#define IDPF_SYNC_WAIT_CNT 1000
static int
...
int idpf_vport_ctlq_add(struct idpf_hw *hw,
			struct idpf_ctlq_create_info *qinfo,
			struct idpf_ctlq_info **cq)
{
	return idpf_ctlq_add(hw, qinfo, cq);
}

void idpf_vport_ctlq_remove(struct idpf_hw *hw,
			   struct idpf_ctlq_info *cq)
{
	idpf_ctlq_remove(hw, cq);
}

int idpf_vport_ctlq_send(struct idpf_hw *hw,
			 struct idpf_ctlq_info *cq,
			 u16 num_q_msg,
			 struct idpf_ctlq_msg q_msg[])
{
	return idpf_ctlq_send(hw, cq, num_q_msg, q_msg, 0);
}

int idpf_vport_ctlq_send_sync(struct idpf_hw *hw,
			 struct idpf_ctlq_info *cq,
			 u16 num_q_msg,
			 struct idpf_ctlq_msg q_msg[])
{
	return idpf_ctlq_send(hw, cq, num_q_msg, q_msg, IDPF_SYNC_WAIT_CNT);
}

int idpf_vport_ctlq_clean_sq(struct idpf_ctlq_info *cq, u16 *clean_count,
			     struct idpf_ctlq_msg *msg_status[])
{
	return idpf_ctlq_clean_sq(cq, clean_count, msg_status);
}
