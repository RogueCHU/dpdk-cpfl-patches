			 ...
			 struct rte_mempool *mp, uint8_t bufq_id)
{
	...
	struct idpf_adapter *base = &vport->itf.adapter->base;
	...
	bufq->queue_id = vport->base.chunks_info.rx_buf_start_qid + queue_idx;
	...
	bufq->qrx_tail = hw->hw_addr + (vport->base.chunks_info.rx_buf_qtail_start +
			 queue_idx * vport->base.chunks_info.rx_buf_qtail_spacing);
		    ...
		    struct rte_mempool *mp)
{
	...
	is_splitq = !!(vport->base.rxq_model == VIRTCHNL2_QUEUE_MODEL_SPLIT);

	rxq->mp = mp;
	rxq->nb_rx_desc = nb_desc;
	rxq->rx_free_thresh = rx_free_thresh;
	rxq->queue_id = vport->base.chunks_info.rx_start_qid + queue_idx;
	...
	if (!is_splitq) {
		...
		rxq->qrx_tail = hw->hw_addr + (vport->base.chunks_info.rx_qtail_start +
				queue_idx * vport->base.chunks_info.rx_qtail_spacing);
		rxq->ops = &def_rxq_ops;
	} else {
...
{
	struct idpf_vport *vport = &CPFL_DEV_TO_VPORT(dev)->base;
	struct idpf_adapter *base = &CPFL_DEV_TO_ITF(dev)->adapter->base;
